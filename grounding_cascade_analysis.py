#!/usr/bin/env python3
"""
Option B: Grounding Error Cascade Analysis for SceneCOT

Measures how often SceneCOT's wrong object grounding causes wrong final answers,
separating "cascade failures" (wrong grounding -> wrong answer) from pure
reasoning failures (right grounding -> wrong answer) and lucky guesses
(wrong grounding -> right answer).
"""
...
def main():
    ...
    ap.add_argument('--types', nargs='+', default=['counting', 'existence'])
    ap.add_argument('--threshold', type=float, default=GROUND_THRESHOLD)
